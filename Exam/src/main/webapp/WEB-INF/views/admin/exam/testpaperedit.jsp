...
function addtpm1() {
	...
	var questionaddlist = $("#sendquiz").val(); // 선택된 selectbox option 배열
 	...
 	tempdata += "tpmdiv=" + tpmdiv + "&tpmSeq=" + tpmSeq + "&cnt=" + questionaddlist.length;
 	
 	location.href="/exam/admin/testpaperquestioneditok.action?"+tempdata;
			...
			<div id="article">
				...
				<table class="table">
						<tr>
							...
							<td>
							<select id="tpbdiv" name="tpbdiv" onchange="tpbdivsel();" class="form-control" style="width: 30%; display: inline-block;">
									<option selected="selected" value="${dto.tpbdivSeq}">${dto.tpbdiv}(선택중)</option>
							...
							<select id="tpmdiv" name="tpmdiv" onchange="tpmdivsel();" class="form-control" style="width: 30%; display: inline-block;">
									<option selected="selected" value="${dto.tpmdivSeq}">${dto.tpmdiv}(선택중)</option>
							</select> 
							<select id="tpsdiv" name="tpsdiv" onchange="tpsdivsel();" class="form-control" style="width: 30%; display: inline-block;">
									<option selected="selected" value="${dto.tpsdivSeq}">${dto.tpsDiv}(선택중)</option>
							</select></td>
						</tr>
						<tr>
							...
							<td><input type="text" class="form-control"
								...
								class="form-control" style="width: 90%;" value="${dto.tpmSubject}"></td>
						</tr>
						<tr>
							<th style="width: 15%; padding-left:25px;">금액</th>
							<td><input type="number" id="testprice" name="testprice"
								class="form-control" style="width: 50%; display: inline-block;" value="${dto.tpmPrice}">원
							</td>
						</tr>
						<tr>
							...
							<td style=""><input type="number" id="testtime"
								...
								style="width: 50%; display: inline-block;" value="${dto.tpmTime}">분</td>
						</tr>

					</table>
					
					<div style="padding-left:25px;">
						<input type="button" value="수정하기" id="testpapermake"
							name="testpapermake" class="btn btn-default"
							onclick="viewquestion();">
						<input type="button" value="목록보기" id="testpapermake1"
							name="testpapermake1" class="btn btn-default"
							onclick="location.href='/exam/admin/testpaperlist.action'">
					</div>
					...
					<form method=get name=FORM>
								...
								<td >
									<div style="min-width: 40%; display: inline-block;">
										<select multiple="multiple" size="10" id="selquiz"
											...
											<c:forEach items="${tpbdivlist2}" var="tpbdivlistdto1">
												<option value="${tpbdivlistdto1.seq}">${tpbdivlistdto1.questionCode}</option>
											</c:forEach>
											
										</select>
									</div> 
									<div style="min-width: 10%; display: inline-block; text-align: center; vertical-align: middle;" >
									...
									<div style="min-width: 40%; display: inline-block;"">
										...
										<c:forEach items="${tpbdivlist1}" var="tpbdivlistdto">
											<option value="${tpbdivlistdto.seq}">${tpbdivlistdto.questionCode}</option>
										</c:forEach>
										</select>
									</div>
						...
						<input type="button" value="수정하기" id="addtpm" name="addtpm"
							class="btn btn-default" onclick="addtpm1();" />
					</form>
