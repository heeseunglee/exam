	...
	<div id="mainBlock">
		...
		<div id="content">
			...
			<!-- #content_body 박스 : 이하 본문 작성하시면됩니다.  -->	
			 ...
			 <table class="table">
			     ...
			     <tr>
			     	<td colspan="2">
			     		<input type="button" value= "뒤로가기" onclick="location.href='/exam/user/freeboardlist.action?seq=${dto.seq}';"/>
			     		<c:if test="${check}">
			     		<input type="button" value= "삭제" onclick="location.href='/exam/user/freeboarddel.action?seq=${dto.seq}';"/>
			     		<input type="button" value= "답글" onclick="location.href='/exam/user/freeboardadd.action?mode=reply&thread=${dto.thread}&depth=${dto.depth}';"/>
			     		<input type="button" value= "수정" onclick="location.href='/exam/user/freeboardedit.action?seq=${dto.seq}';"/>
			     		</c:if>
			     	</td>
			     </tr>
			 </table>  
			  ...
			  <!--댓글 쓰기  -->
				<form method="POST" action="/exam/user/freeboardcomment.action?pSeq=${dto.seq}">
		...
		<!-- 목록 -->
			<table class="table table-striped">
				...
				<tr>
					<td>
						<a href="/exam/admin/addms.action?memberId=${cdto.userId}">
							${cdto.name}
						</a>
					</td>
					<td>${cdto.content}</td>
					...
					<td>
						...
						<input type="button" value="삭제하기" class="form-control" onclick="location.href='/exam/user/freeboardcommentdel.action?seq=${cdto.seq}&pSeq=${dto.seq}';" />
						</c:if>
					</td>
				</tr>
				</c:forEach>
